...
class ProjectListFragment : Fragment() {
    ...
    fun projectUpdated(project: Project){
        val i = projectList.indexOfFirst { it.projectID == project.projectID }
        if(i != -1){
            projectList[i] = project
            recyclerView.adapter?.notifyItemChanged(projectList.indexOf(project))
        }
    }
