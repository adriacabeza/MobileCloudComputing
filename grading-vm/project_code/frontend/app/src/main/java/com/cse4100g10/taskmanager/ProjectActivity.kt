...
class ProjectActivity : AppCompatActivity() {
        ...
        var urlTask = uploadTask?.continueWithTask { task ->
            ...
            if(task.isSuccessful){
                ...
                if(isImage) {
                    ...
                    (pagerAdapter.getPicturesFragment() as ProjectPicturesFragment).showLoading()
                }else {
                    Snackbar.make(findViewById(R.id.cc_root), R.string.file_upload_success, Snackbar.LENGTH_SHORT).show()
                    (pagerAdapter.getFilesFragment() as ProjectFilesFragment).showLoading()
                }
                getAll()
        ...
        val req = JsonObjectRequest(
            ...
            Response.Listener { response ->
                ...
                for (i in 0 until tasksResponse.length()) {
                    ...
                    tasksList.add(Task(item["id"] as String, item["name"] as String, item["description"] as String, item["status"] == "completed", item["assigned_to"] as JSONArray))
                }
    ...
    private fun deleteTask(task: Task) {
        val projectAPI = "https://mcc-fall-2019-g10.appspot.com/api/project/${project.projectID}/tasks"

        val jsonarray = JSONArray()
        jsonarray.put(task.taskID)
        val que = Volley.newRequestQueue(this)
        val req = MyJsonObjectRequest(
            Request.Method.GET, projectAPI,jsonarray,
            Response.Listener {
                    response ->
                    // response == projectID
            }, Response.ErrorListener {
